...
chat_client_connect(struct chat_client *client, const char *addr)
{
    ...
    int flagMask = fcntl(sockDesc, F_GETFL, 0);
    flagMask |= O_NONBLOCK;

    if(fcntl(sockDesc, F_SETFL, flagMask) < 0){
        freeaddrinfo(result);
        free(temp_addr);
        return CHAT_ERR_SYS;
    }
...
chat_client_update(struct chat_client *client, double timeout)
{
    ...
    if(copyFd.revents & POLLIN){
        char buffer[1024];
        int recieved;
        if ((recieved = recv(client->socket, buffer, sizeof (buffer), 0)) < 0){
            return CHAT_ERR_SYS;
        }else{
...
chat_client_feed(struct chat_client *client, const char *msg, uint32_t msg_size)
{
    ...
    for(uint32_t i = 0; i < msg_size; ++i){
        ...
        if(msg[i] == '\n') {
            ...
            for(; end >= 0 && (currentMessage[end] == '\0' || isspace(currentMessage[end])) ; --end);
            if (end < 0) {
                return 0;
            }
